defmodule AwesomeShopWeb.APIAuthPlug do
  @moduledoc false
  use Pow.Plug.Base
  ...
  def create(conn, user, config) do
    ...
    access_token = sign_token(conn, user.email, max_age: @access_token_max_age)
    renewal_token = sign_token(conn, user.email, max_age: @renewal_token_max_age)

    conn =
      conn
      |> Conn.put_private(:api_access_token, access_token)
      |> Conn.put_private(:api_renewal_token, renewal_token)
